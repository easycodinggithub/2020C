...
#include <conio.h>
#include <mmsystem.h> // playsound 함수 사용, 음악 재생시 필요
...
// 단어 Basic, Mid, High 정의
// _Word : 단어
// _Meaning : 뜻
// _EXample_Word : 예문 문장
// _Example_Meaning : 예문 해석
// _Proverb_Word : 속담 문장
// _Proverb_Meaning : 속담 해석

// 현재 Basic 에 1000개이고, Mid, High 는 아직 단어가 다 입력 X 
...
void console_window(); // 콘솔 창 크기 및 위치 조정
void console_color_change(int text_color, int background_color); // 글자 색 변경 함수

char choice_BMH(char* pointer_BMH); // BMH 선택 함수
int choice_mode(int* pointer_MODE); // MODE 선택 함수
int choice_day(int* pointer_DAY, int check_MODE); // DAY 선택 함수
int choice_level(int* pointer_LEVEL); // LEVEL 선택 함수

void Designed(int); // "=" 출력 함수
void Print(int); // 반복되는 문구 정리
void Print_All(int, int, int); // 반복되는 문구 정리
int change_song(int*); // SONG 선택 함수
void gotoxy(int, int); // 하단 문구 출력 함수

typedef struct Menu M; // 전체 기본 설정
typedef struct Menu1 M_1; // MODE 1에 쓰일 구조체
typedef struct Menu2_3_4 M_2_3_4; // MODE 2,3,4 에 쓰일 구조체
...
int main() {
	// 시간을 사용하기 위한 정의
	time_t timer; // 초단위의 시간을 정수값으로 표현하는 데이터 ( 정리가 필요함 )
	struct tm* t; // tm 구조체, timer 을 정렬해줌
	...
	while (1) {
		FILE* Musicr = NULL; // 현재 Music 읽기 
		FILE* Musicw = NULL; // 현재 Music 쓰기
		FILE* Scorer = NULL; // HangMan Game 최고 기록 읽기
		FILE* Scorew = NULL; // HangMan Game 최고 기록 쓰기

		// HangMan 최고기록 받아와서 그 값 ( HangMan_History )에 따라 등급 출력
		...
		srand((unsigned)time(NULL)); // Random 수 사용 
		console_window(); // 콘솔 기본 설정 함수
		console_color_change(4, 15); // 기본 색 설정
		gotoxy(6, 36); // 콘솔 창에 다른 위치로 이동 함수
		printf("     [현재 HANG MAN GAME 최고 기록은 : %d 점]     [당신은 현재 <%s> 수준 입니다.]     [%d 년 %d 월 %d 일]", M1.HH, LEVEL_PRINT[M1.GL], t->tm_year + 1900, t->tm_mon + 1, t->tm_mday); 
		gotoxy(0, 0); // 콘솔 창에 다른 위치로 이동 함수
		Print_All(0, 1, 2);
		console_color_change(0, 15);
		choice_BMH(&M1.BMH); // BMH 의 주소를 전달 
		if (M1.BMH == 'B') { // Basic 을 선택시 단어장 Basic 공부
		...
		else if (M1.BMH == 'M') { // Mid 을 선택시 단어장 Mid 공부
		...
		else if (M1.BMH == 'H') { // Hell 을 선택시 단어장 Hell 공부
		...
		else if (M1.BMH == 'Q') { // Q 를 선택시 음악 변경
			change_song(&M1.SONG);
		...
		else if (M1.BMH == 'X') { // X 를 선택시 프로그램 종료
			exit(0);
		}
		else {
			Print(1);
		...
		choice_mode(&M1.MODE);
		if (M1.MODE == 1) { // MODE 가 1일때 => LOOK MODE 일때
			M_1 M2;
			...
			while (1) {
				if (_kbhit()) {               // 1. while문안에서 키보드 눌렸을 시 if문이 실행된다.
					...
					if (M2.Arrow == -32) {           // 3. -32로 입력되면
						...
						if (M2.Look_Page <= -1 || M2.Look_Page >= 20) { // 현재 보려고 하는 페이지가 -1페이지 이거나 20페이지 일때 ( 양쪽 끝페이지 일때 )
							system("cls");
							Print_All(0, 3, 2);
							printf("   < DAY %d - 중요 속담 ! > \n\n\n", M1.DAY);
						...
						else { // 현재 보려고 하는 페이지가 1페이지 ~ 19 페이지 일때
							system("cls");
							Print_All(0, 3, 2);
							printf("   < DAY %d - %d > \n\n\n", M1.DAY, M2.Look_Page + 1);
							printf("   - %s : ", Array_Word[M1.DAY - 1][M2.Look_Page]);
		...
		}else if (M1.MODE == 2) { // MODE 가 1일때 => REVIEW MODE 일때
			PlaySound(NULL, 0, 0); // 배경 노래 OFF 효과음이 나와야 함으로
			M_2_3_4 M3;
			M3.Review_O = 0, M3.Review_X = 0, M3.RW = 1;
			choice_day(&M1.DAY, M1.MODE); // REVIEW 할 DAY 선택
			choice_level(&M1.LEVEL); // LEVEL 선택 ( NORMAL, HARD, HELL )
			Designed(0);
			printf("================================================== < REVIEW MODE > DAY %d REVIEW =================================================\n", M1.DAY);
			Designed(2);
			clock_t start_1 = clock(); // 시작 시간 저장
			if (M1.LEVEL == 1 || M1.LEVEL == 2) { // LEVEL 이 NORMAL OR HARD 일때, 즉 선택형 복습일때
				for (int i = 0; i < 20; i++) {
					M3.R1 = rand() % 4; // 1, 2, 3, 4 중 한개 자리 선택
					if (M1.LEVEL == 1) { // 단어 문제이면
...
void Print(int a) {
	...
	case 35: printf("======================================= 보기를 원하는 DAY 를 입력해 주세요. ( 1 ~ 50 DAY ) =======================================\n"); break;
	case 36: printf("======================================= 복습를 원하는 DAY 를 입력해 주세요. ( 1 ~ 50 DAY ) =======================================\n"); break;
	}
}
